...
import ConsoleApp.CarRentalSystem.DriverCarRentalApp;
...
public class Customer extends User{
	...
	public String toString() {
	    ...
	    String formattedEmail = String.format("%-24s", email_Address);
	    String formattedPhoneNumber = String.format("%-12s", phoneNumber);
	    String formattedAge = String.format("%-3d", age);
	    String formattedAddress = String.format("%-16s", address);
	    String formattedLicenseNumber = String.format("%-15d", license_number);
	    String formattedGender = String.format("%-6s", gender);

	    return String.format("%-6d | %s | %s | %s | %s | %s | %s | %s | %s",
	            custid, formattedFirstName, formattedLastName, formattedEmail, formattedPhoneNumber, formattedAge, formattedAddress, formattedLicenseNumber, formattedGender);
	}
	...
	public void customerEntry() throws InvalidChoiceException, InvalidValidationException {
        ...
        switch(op) {
	        ...
	        default:
	        	try {
		        	throw new InvalidChoiceException("\t\tYour choice is not allowed !! please provide values from 1-3 !!");
	...
	public void customerLogin() throws InvalidChoiceException, InvalidValidationException {
    	...
    	String pd="";
    	try {
    		System.out.print("\t\tUser ID:");id=sc.nextInt();
    	...
    	try {
			...
			ResultSet rs=st.executeQuery("Select * from car.Login_credentials");
			 while (rs.next()) {
		            ...
		            loginCredentialsList.add(new Account(userId, userPassword, userRole));
//		            System.out.println(userId+" "+userPassword+" "+userRole);
		     }
			 for (Account loginCredentials : loginCredentialsList) {
			        if (loginCredentials.getUserid() == id && loginCredentials.getPassword().equals(pd) && loginCredentials.getRole().equals("customer") && loginCredentials.getStatus().equals("ACTIVE")) {
			            flag=1;
			        }
			  }
		}catch(Exception e){
    	...
    	if(flag==1)
    	{
    		System.out.println("\t\t----> Your have logged in successfully <-------");
    		while(true)
    		{
    			System.out.print("\t**********************************************************************************************\n");
    	        System.out.print("\t*                                                                                            *\n");
    			...
    			switch(ch)
    			{
    				...
    				{
    					try {
    					throw new InvalidChoiceException("Your choice is not allowed !! please select between 1-8!!");
    					}catch(InvalidChoiceException e) {
    						System.out.println(e.getMessage());
    					}
    				}
